...
// should record the last location that subgroup matched at, even if these locations
// are within different matches of group A. For example, in the following regexps
// group `(\s+)` matches ' ' and group `(\w+)` matches 'test', even though
// their common parent group matches 'test' (without the space).
